...
import { X, Upload, MessageSquareWarning } from 'lucide-react'
...
export default function ReportModal({ isOpen, onClose, appointmentId, onSuccess }: ReportModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // We need to use apiClient directly or add createReport to service
      // Let's add createReport to bookingChecklistService
      ...
      showError(err.response?.data?.message || 'Lỗi khi gửi khiếu nại')
    } finally {
      ...
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
          ...
          <form id="report-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full px-3 py-2 border border-slate-200 rounded-lg text-sm outline-none focus:border-indigo-500 min-h-[100px] resize-none"
              />
            </div>

            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">Hình ảnh bằng chứng (Tối đa 5 ảnh)</label>
        ...
        <div className="px-6 py-4 border-t border-slate-100 flex justify-end gap-3 bg-slate-50">
          ...
          <button
            ...
            form="report-form"
            disabled={isSubmitting}
            className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-lg shadow-sm transition flex items-center gap-2 disabled:opacity-70"
          >
